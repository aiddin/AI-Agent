      ...
      <div class="p-4 lg:p-6 space-y-4 lg:space-y-6">
        ...
        <div class="panel">
          ...
          <div class="space-y-4">
            ...
            <!-- Image Preview -->
            <div class="upload-area border-2 border-dashed border-[#ebedf2] dark:border-[#191e3a] rounded-lg overflow-hidden transition-all duration-300 hover:border-primary hover:bg-primary/5">
              <div v-if="!imageUrl && !isLoading" class="flex flex-col items-center justify-center p-8 text-center">
                <icon-gallery class="w-12 h-12 text-white-dark mb-2" />
                <span class="text-white-dark">Your receipt image will appear here</span>
              </div>
              <img v-if="imageUrl" :src="imageUrl" alt="Uploaded Receipt" class="w-full h-48 object-cover" />
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="panel border-success bg-success-light dark:bg-success/10">
            <div class="flex items-center">
              <div class="w-12 h-12 bg-success text-white rounded-full flex items-center justify-center ltr:mr-4 rtl:ml-4">
          ...
          <div class="panel border-danger bg-danger-light dark:bg-danger/10">
            <div class="flex items-center">
              <div class="w-12 h-12 bg-danger text-white rounded-full flex items-center justify-center ltr:mr-4 rtl:ml-4">
          ...
          <div class="space-y-4">
            <div v-for="(item, index) in items" :key="index" class="panel border-[#e0e6ed] dark:border-[#1b2e4b]">
              <div class="space-y-4">
                ...
                <!-- Remark -->
                <div v-if="item.Remark && item.Remark !== ''" class="bg-[#f1f2f3] dark:bg-[#1b2e4b] rounded-lg p-4">
                  <label class="font-medium text-white-dark">Remark:</label>
                  <p class="dark:text-white-light mt-1">{{ item.Remark }}</p>
                </div>

                <!-- Reasoning -->
                <div v-if="item.Reasoning && item.Reasoning !== ''" class="bg-[#f1f2f3] dark:bg-[#1b2e4b] rounded-lg p-4">
